window.onload = function () {
    renderGestoresPendentes();
}

async function renderGestoresPendentes() {
    const response = await fetch('http://localhost:8080/GestorAdmins/');
    const gestores = await response.json();
    const bodyTabela1 = document.getElementById("aceitarparceiros");
    bodyTabela1.innerHTML = "";

    for (gestor of gestores) {
        if (gestor.Ativo.data[0] == 1 && gestor.Resultado == 0 && gestor.Tipo == 2) {
            ...
            newRow.innerHTML += '<td class="pt-3-half" contenteditable="false">' + data.getDate() + '/' + data.getMonth() + 1 + '/' + data.getFullYear() + " </td> ";
            newRow.innerHTML += '<td class="pt-3-half" contenteditable="false"> <button type="button" class="btn btn-primary" onclick = renderModal("' + gestor.idGestor_Admin + '")>Ver Mais</button>';
        }
    }
}
...
async function renderModal(id) {
    ...
    for (espaco of espacos) {
        ...
        if (isIterable(horarios)) {
            ...
            hora_abertura = "Não definida";
            hora_encerramento = "Não definida"
        }
        ...
        modal.innerHTML += '<div style="margin-left: 65%"> <button type="button" class="btn btn-primary" onclick = aceitar("' + espaco.idGestor_Admin_FK9 + '")>Aceitar</button> <button type="button" class="recusar-b" onclick = recusar("' + espaco.idGestor_Admin_FK9 + '")>Recusar</button> </div>'
        }
    }
}

function aceitar(id) {
    var confirmar = confirm("Tem a certeza que pretende aceitar este Parceiro?");
    if (confirmar) {
...
function recusar(id) {
    var confirmar = confirm("Tem a certeza que pretende recusar este Parceiro ?");
    if (confirmar) {
...
async function alterarResultado(id, value) {
    ...
    if (value) {
        url = "http://localhost:8080/GestorAdmins/" + id + "/aceitar"
        alerta = "Parceiro Aceite com sucesso";
    } else {
        url = "http://localhost:8080/GestorAdmins/" + id + "/recusar"
        alerta = "Parceiro Recusado com sucesso";
    }
    fetch(url, {
        headers: {
